...
class AccountDetailsAdminForm(forms.ModelForm):
    currency_select = forms.ChoiceField(
        required=False,
        choices=[('', 'Choose from list…')] + list(AccountDetails.ACCOUNT_CURRENCY),
        label='Select Currency'
    )
    currency_custom = forms.CharField(
        required=False,
        label='Or Enter Custom Currency',
        widget=forms.TextInput(attrs={'placeholder': 'e.g. XYZ'})
    )

    class Meta:
        model = AccountDetails
        exclude = (
            'gender', 'account_type', 'day', 'month', 'year',
            'bitcoins', 'ethereums', 'usdt_trc20s', 'trons',
            'account_currency',
        )

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.instance and self.instance.account_currency:
            currency_codes = [choice[0] for choice in AccountDetails.ACCOUNT_CURRENCY]
            if self.instance.account_currency in currency_codes:
                self.fields['currency_select'].initial = self.instance.account_currency
            else:
                self.fields['currency_custom'].initial = self.instance.account_currency

    def clean(self):
        cleaned_data = super().clean()
        selected = cleaned_data.get('currency_select')
        custom = cleaned_data.get('currency_custom')

        if custom:
            self.cleaned_currency = custom.strip()
        elif selected:
            self.cleaned_currency = selected
        else:
            raise forms.ValidationError("Please select or enter a currency.")

        return cleaned_data

    def save(self, commit=True):
        instance = super().save(commit=False)
        # Save cleaned currency value to the model field
        instance.account_currency = self.cleaned_currency
        if commit:
            instance.save()
        return instance
...
class AccountDetailsAdmin(admin.ModelAdmin):
    ...
    exclude = (
        'gender', 'account_type', 'day', 'month', 'year',
        'bitcoins', 'ethereums', 'usdt_trc20s', 'trons',
        'account_currency',  # must exclude from admin or it will show as extra field
    )
